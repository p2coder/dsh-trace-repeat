...
function mapEventToVersion(event, info) {
	...
	if (type === "task-control/paused") return null;
	if (type === "task-control/resumed") return null;
	return null;
}
...
function reconcilePause(ctx, traceRoot, config, state, sessionId) {
	...
	if (pausedNow) {
		...
		appendMarkerVersion(traceRoot, config, state, sessionId, "paused", {
	...
	appendMarkerVersion(traceRoot, config, state, sessionId, "resumed", {});
}
/** Append one marker version (paused / resumed) and git-commit it when enabled. */
function appendMarkerVersion(traceRoot, config, state, sessionId, type, detail) {
